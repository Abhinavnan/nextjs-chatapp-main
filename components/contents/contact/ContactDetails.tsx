...
const componentStyle: Record<string, string> = {
    ...
    chat: "flex flex-row md:flex-col w-full md:w-60 lg:w-100 gap-4 md:gap-8 p-2 md:p-4 bg-white shadow-md rounded-md h-fit md:h-auto"
};
        ...
        <div className={componentStyle[type]}>
            ...
            <div className={`flex flex-col gap-1 ${chatType && 'flex-col md:flex-col'}`}>
                ...
                <Tooltip text={contactData.email}>
                    <p className={cn("text-sm text-gray-500", chatType ? "block lg:hidden" : "hidden")}>
                        {shrinkText(contactData.email, 20)}
                    </p>
                </Tooltip>
                <p className={`text-md text-gray-700 ${chatType && `hidden md:block`}`}>{contactData.about}</p>
