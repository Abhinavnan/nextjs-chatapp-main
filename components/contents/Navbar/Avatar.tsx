...
const Avatar = ({ profilePicture }: { profilePicture: string }) => {
  ...
  return (
    <div className="dropdown dropdown-end relative">
      ...
      {isOpen && (
        <ul tabIndex={-1}
          className="absolute right-0 mt-2 p-2 shadow menu dropdown-content bg-olive-50 rounded-box dark:bg-mist-950">
          <li className={hideProfileButton ? "hidden" : ""}>
            <a className="justify-between dark:bg-mist-900! dark:hover:bg-mist-800!" href="/profile">
              Profile
            </a>
          </li>
          <div className={hideProfileButton ? "hidden" : "border-t border-gray-200 my-1 dark:border-mist-700"} />
          <li>
            <button onClick={handleLogout} className="cursor-pointer justify-between dark:bg-mist-900! dark:hover:bg-mist-800!">Logout</button>
          </li>
        </ul>
      )}
    </div>
