        ...
        <form onSubmit={handleSubmit}
         className="grid grid-cols-1 place-items-center gap-1 w-[calc(100%-2rem)] md:w-96 p-6 bg-white rounded-lg shadow-md m-2">
            ...
            {checkField('oldPassword') && <Link href="/reset-password?status=new" 
              className="text-sm text-green-600 hover:underline w-full flex justify-end mt-[-8]">
                forgot password?
            </Link>}
            ...
            {enableImageUpload && <div className="flex flex-col w-full items-center">
                <button type="button" onClick={() => fileInputRef.current?.click()} disabled={isLoading}
                 className="bg-sky-500 text-white p-2 mt-2 rounded hover:bg-sky-600 w-full flex disabled:opacity-50">
            ...
            {(imageUrl || values.profilePicture) && <button type="button" disabled={isLoading} onClick={handleRemoveProfilePicture}
              className="bg-red-500 text-white p-2 mt-2 rounded hover:bg-red-600 w-full flex disabled:opacity-50">
                <ImageMinus />
                <p className="flex-1 text-center">Remove Profile Picture</p>
            </button>}
            <button type="submit" disabled={isLoading} 
              className="bg-green-500 text-white p-2 mt-2 rounded hover:bg-green-600 w-full disabled:opacity-50">
                {primaryButtonText[type]}
            </button>
            <Link  href={secondaryButtonLinks[type]} 
             className={cn("border border-green-500 text-green-500 p-2 mt-2 rounded text-center", 
                "hover:border-green-600 w-full hover:text-green-600 hover:bg-green-100")}>
